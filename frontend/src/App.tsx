...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <Navbar />
      <div className="bg-gray-900 text-white p-2 text-center text-sm flex justify-center items-center gap-3 z-30 relative">
        <span className="font-mono text-gray-300">TRUSTRAIL ACTIVE:</span>
        <input type="text" placeholder="Paste Mandate ID here (e.g. man_170000)" className="bg-gray-800 border border-gray-700 rounded px-3 py-1 w-64 text-white focus:outline-none focus:border-red-500" value={mandateId} onChange={e => setMandateId(e.target.value)} />
      </div>
      <ProductGrid catalog={catalog} />
      
      {/* Live Audit Trail - floating button + panel, mirrors chatbot pattern */}
                ...
                auditLogs.map((log, i) => (
                  <div key={i} className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
                    ...
                    <div className="p-3 space-y-3">
                      ...
                      {/* Evaluated — the key "Code Disposes" proof */}
                      {log.evaluated && (
                        <div>
